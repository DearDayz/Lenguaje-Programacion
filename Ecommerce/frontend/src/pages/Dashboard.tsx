...
import { FileText, User, Clock, LogOut, Home, Edit, Download } from 'lucide-react';
            ...
            <Tabs defaultValue="profile" orientation="vertical" className="w-full">
              <TabsList className="flex flex-col items-start justify-start w-full space-y-1">
                ...
                >
                  ...
                  <span className="text-sm md:text-base">Historial de Pedidos</span>
                </TabsTrigger>
              </TabsList>
            </Tabs>
          </CardContent>
                  ...
                  {purchaseHistory.length > 0 ? (
                    <div className="divide-y">
                      {purchaseHistory.map((purchase) => (
                        <div key={purchase.id} className="py-4 flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                          <div>
                            <h3 className="font-medium">Factura: {purchase.id}</h3>
                            <div className="flex items-center text-sm text-gray-500">
                              <Clock className="mr-1 h-3 w-3" />
                              {purchase.date}
                            </div>
                            <p className="text-sm">
                              {purchase.items} {purchase.items === 1 ? 'artículo' : 'artículos'}
                            </p>
                          </div>
                    ...
                    <div className="text-center py-8">
                      ...
                      <h3 className="mt-2 text-lg font-medium">No tienes compras</h3>
                      <p className="text-gray-500 mt-1">Tu historial de compras aparecerá aquí.</p>
